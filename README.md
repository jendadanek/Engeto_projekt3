...
```if URL in Zlínský:
    ...
    Øád_národa.append(tabulka2[17])
    CESTA_ODPOVÌDNÉ_SPOLEÈNOSTI.append(tabulka2[22])
    ÈSSD.append(tabulka2[27])
    Cibulka.append("nekandiduje")
    Radostné_Èesko.append(tabulka2[32])
    STAN.append(tabulka2[37])
    KSÈM.append(tabulka2[42])
```
